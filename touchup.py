...
archive = os.listdir('C:\\Users\\aksha\\PycharmProjects\\breastcancer\\IDC_regular_ps50_idx5')
m = 0
for i in range(len(archive)):
    Id = os.listdir('C:\\Users\\aksha\\PycharmProjects\\breastcancer\\IDC_regular_ps50_idx5\\' + archive[i] + '')
    Ids = archive[i]

    for j in range(len(Id)):

        sample = os.listdir('C:\\Users\\aksha\\PycharmProjects\\breastcancer\\IDC_regular_ps50_idx5\\' + archive[i] + '\\' + Id[j] + '')
        d = Id[j]
        for k in range(len(sample)):
            ...
            data = [[Ids,d,radius_mean,perimeter_mean,area_mean,compactness,concavity_points,cx,cy,area,perimeter,concavity_mean,concave_points,fractal_dimension,x_mean,y_mean,radius_c,contours_circle,mean,std,smoothness]]
            df = pd.DataFrame(data, columns=['Id','diagnosis','radius_mean','perimeter_mean','area_mean','compactness','concavity_points','cx','cy','area','perimeter','concavity_mean','concave_points','fractal_dimension','x_mean','y_mean','radius_c','contours_circle','mean','std','smoothness'])

            frames = [add_df, df]
            add_df = pd.concat(frames)
            add_df = add_df.reset_index(drop=True)
            add_df.to_csv('breastcancerdata.csv')
print(add_df.head())
